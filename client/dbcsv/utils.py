...
from dbcsv.exception import OperationalError

def query(url: str, token: str, schema: str, query: str) -> Iterator[List[Any]]:
    header = {
    ...
    try:
        r = requests.post(f"{url}/query/sql/", json=data, headers=header, timeout=300, stream=True)        
        r.raise_for_status()
        for line in r.iter_lines(decode_unicode=True):
            if line:
                try:
                    ...
                    raise OperationalError(f"Invalid JSON received: {line}")
        ...
        try:
            ...
            detail = str(exc)
        raise OperationalError(f"HTTP Error: {detail}")
    
    except requests.RequestException as exc:
        raise OperationalError(f"Request failed: {str(exc)}")
